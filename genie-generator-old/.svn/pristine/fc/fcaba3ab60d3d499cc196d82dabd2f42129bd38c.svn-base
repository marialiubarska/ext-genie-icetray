...
#
#  Usage: 
# subm_gicevgen.sh flavor runnum_start nr_runs emin emax expon
#
if [ $# -ne 6 ]
then
...
jobname=$MYGENIE/pbs/genie_icetray_pbs.job

for i in `seq ${runnum_start} ${runnum_end}`; do
    export runnum=$i
    varlist="runnum,flavor,emin,emax,expon"

    qsub -d `pwd` -q localcluster -l walltime=2:00:00 -v $varlist $jobname
    #qsub -d `pwd` -q localcluster -l walltime=2:00:00  $genie_job
